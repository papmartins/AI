...
class AnomalyDetector
{
    ...
    protected function prepareDataset(): void
    {
        ...
        $csvData = [];
        $csvData[] = ['user_id', 'rentals_per_day', 'avg_rental_duration', 'rating_deviation', 'return_delay_percentage'];
    ...
    protected function determineAnomalyType(User $user, float $score): string
    {
        ...
        $lateReturns = 0;
        foreach ($user->rentals as $rental) {
            if ($rental->returned_at && $rental->due_date && $rental->returned_at > $rental->due_date) {
                $lateReturns++;
            }
        }
        
        if ($user->rentals->count() > 0 && ($lateReturns / $user->rentals->count()) > 0.3) {
            return 'frequent_late_returns';
        }
